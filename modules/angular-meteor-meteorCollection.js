...
AngularMeteorCollection.prototype.updateCursor = function (cursor) {
  ...
  function safeApply() {
    // If already updating from the server, then digest is already timeouted and will run in next turn
    ...
    // Making sure we are running digest only once, after all changes are handled (in next turn)
    $timeout(function() {
