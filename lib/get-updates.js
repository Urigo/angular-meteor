...
(function() {
  ...
  var getDifferenceForSetUnsetPull = (function () {
    var getDifferenceForSetUnsetPull = function (src, dst, isShallow) {
      var level;
    ...
    var compare = function(src, dst) {
      ...
      return keys.reduce(function(diff, k) {
        ...
        if (_.isObject(srcValue) && _.isObject(dstValue)) {
          var valueDiff = getDifferenceForSetUnsetPull(srcValue, dstValue);
          utils.setFilled(diff, k, valueDiff);
        }

        // Need to use $push when srcValue is undefined and dstValue is a nonempty array.
        else if (!(srcValue === undefined && _.isArray(dstValue) && dstValue.length > 0) && srcValue !== dstValue) {
          diff[k] = dstValue;
        }
    ...
    return getDifferenceForSetUnsetPull;
  })();


  var getDifferenceForPush = (function () {
    var getDifferenceForPush = function (src, dst, isShallow) {
      var level;
    ...
    var compare = function (src, dst) {
      ...
      return keys.reduce(function (diff, k) {
        ...
        if (_.isObject(srcValue) && _.isObject(dstValue)) {
          var valueDiff = getDifferenceForPush(srcValue, dstValue);
          utils.setFilled(diff, k, valueDiff);
        }

        if (srcValue === undefined && _.isArray(dstValue) && dstValue.length > 0) {
          diff[k] = dstValue;
        }
  ...
  var getUpdates = (function () {
    var getUpdates = function (src, dst, isShallow) {
      ...
      var diff = getDifferenceForSetUnsetPull(src, dst, isShallow);
      ...
      diff = getDifferenceForPush(src, dst, isShallow);
      paths = utils.toPaths(diff);

      var push = createPush(paths);
    ...
    var createPush = function (arr) {
      return _.reduce(arr, function (result, v, k) {
